...
Se usa un **cliente OAuth de Tailscale** (secrets `CLIENTID`/`CLIENTSECRET` del Environment `prod`) en vez de un auth-key de larga duración: el cliente OAuth genera credenciales efímeras por ejecución y se puede revocar/rotar desde el panel de Tailscale sin tocar ningún secret de GitHub; un auth-key reusable, si se filtrara, seguiría siendo válido hasta su expiración fija. El cliente OAuth se restringe a un tag dedicado (`tag:ci-deploy`), con una política ACL de Tailscale que solo permite a ese tag alcanzar el servidor de producción por el puerto 22 — ningún otro dispositivo de la tailnet, ningún otro puerto.

**Hallazgo real durante la verificación**: el scope `Devices → Core` (Read/Write) del cliente OAuth no basta para que un runner efímero se una a la tailnet — se obtiene un `403 calling actor does not have enough permissions to perform this function` en `tailscale up`. Hace falta además el scope `Keys → Auth Keys` (con `tag:ci-deploy` también en su propio selector de tags, independiente del de `Devices Core`), que es el que autoriza generar la clave de pre-autenticación efímera que usa el runner al unirse.
...
El acceso de red (Decisión 1) sigue igual: la política ACL de Tailscale restringe `tag:ci-deploy` al puerto 22 del servidor. Se añade además un bloque `ssh` con `"action":"accept"` (no `"check"`, que exige reautenticación interactiva en navegador y es inviable en CI) que concede a `tag:ci-deploy` una sesión como el usuario `ci-deploy` — sin este bloque, el acceso de red no basta para abrir sesión SSH. El `dst` de ese bloque usa un tag (los bloques `ssh` de Tailscale solo aceptan tag/grupo/autogroup como destino, nunca una IP directa).

**Corrección real durante la verificación end-to-end**: el intento inicial usó `dst: ["tag:ingress"]`, asumiendo que era el tag del propio servidor `debian` — error real: `tag:ingress` resultó pertenecer a los nodos internos de infraestructura de Tailscale Funnel (`funnel-ingress-node`, decenas de ellos), no al servidor. El propio `debian` no llevaba ningún tag. Solución real aplicada: se creó un tag nuevo y dedicado `tag:prod-server` (declarado en `tagOwners`), asignado a `debian` desde el panel de Tailscale (Machines → Edit ACL tags, no vía `tailscale up`/`set` por SSH, que habría podido resetear otros flags ya activos del nodo como `--ssh`/Funnel). Como un dispositivo etiquetado deja de contar como `autogroup:self` para su propietario, se añadió una regla `ssh` adicional en modo `check` (`src: autogroup:member, dst: tag:prod-server`) para preservar explícitamente el acceso SSH humano normal de `gonzalo` a su propio servidor, que de otro modo se habría roto silenciosamente al etiquetarlo.
