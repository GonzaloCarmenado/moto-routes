...
La API SHALL verificar el email y la contraseña recibidos contra la cuenta registrada y, si coinciden, emitir un token de sesión; si no coinciden, SHALL rechazar la petición sin revelar si el email existe o no.

#### Scenario: Login correcto devuelve un token
- **WHEN** un cliente envía el email y la contraseña correctos de una cuenta existente
