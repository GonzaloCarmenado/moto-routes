...
- [x] 1.5 Añadir al grupo `docker` al usuario `[usuario-redactado]` (`usermod -aG docker [usuario-redactado]`) para no depender de `sudo` en cada comando — confirmado con `docker ps` sin `sudo` en una sesión SSH nueva.

## 2. Clonar el repositorio y preparar la base de datos

- [x] 2.1 `git clone https://github.com/crzverde/moto-routes.git` en el servidor (repo público, sin credenciales) — clonado en `/home/[usuario-redactado]/moto-routes`.
...
- [x] 5.2 `curl http://[tailscale-ip-redactada]:8080/api/ping` desde esta máquina (dentro del mismo tailnet) → 200 con timestamp real de `appdb`.
- [x] 5.3 `ss -tlnp` en el servidor → el puerto 8080 solo aparece asociado a `[::ffff:[tailscale-ip-redactada]]`, nunca a `0.0.0.0` ni a `192.168.1.23`.
