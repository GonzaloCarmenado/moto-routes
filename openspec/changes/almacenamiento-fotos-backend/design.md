...
### D6. Nuevo servicio `minio` en CI y en `docker-compose.yml` de desarrollo
- **Decisión**: contenedor `minio/minio` como *service container* en `.github/workflows/ci.yml` (mismo patrón que `postgres`, con `--health-cmd` contra el endpoint de salud de MinIO) y como servicio nuevo en `infra/docker/docker-compose.yml` para desarrollo local — el test de `BlobStore` necesita un MinIO real, igual que `dbtest.Connect` necesita un Postgres real para `internal/routes/`.
- **Alternativas**: fake en memoria de `BlobStore` para los tests — descartado como único mecanismo de test (aunque puede usarse en los tests de `handler.go`, que no necesitan ejercitar el SDK real): un fake no detecta problemas reales de configuración/protocolo con MinIO, mismo razonamiento que ya se aplicó a `internal/routes/` con Postgres real.
