...
- [ ] 8.3 Verificar en el servidor de producción real (mismo patrón que ADR-034/ADR-036: `curl`, sin dejar secretos reales en ningún artefacto) que el enlace de confirmación funciona desde fuera del tailnet (usa `PUBLIC_API_BASE_URL` = URL de Tailscale Funnel).
- [ ] 8.4 Cuenta de prueba usada en 8.2/8.3 limpiada de la base de datos de producción tras verificar (mismo criterio ya seguido en ADR-034 con registro/login de prueba).

## 9. Cierre

- [ ] 9.1 Actualizar `memory/context.md` con el estado del cambio.
- [ ] 9.2 Si la reutilización de `LoginRateLimiter` para un caso no-login (tarea 4.5) se considera confusa en la revisión, anotar el rename pendiente como deuda técnica en `memory/decisions.md` en vez de hacerlo en este cambio.
