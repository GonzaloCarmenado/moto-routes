...
describe('openRegisterDialog', () => {
  ...
  it('envío correcto llama a registerAccount, muestra el mensaje de verificación pendiente y resuelve "registered"', async () => {
    vi.mocked(registerAccount).mockResolvedValue({ id: 1, email: 'rider@example.com' });
    ...
    expect(registerAccount).toHaveBeenCalledWith('http://localhost:8080', 'rider@example.com', 'correct-horse-battery');
  ...
  it('conserva el email y la contraseña escritos tras un error (no los pierde al re-renderizar)', async () => {
    vi.mocked(registerAccount).mockRejectedValue(new AuthApiError('email-taken', 'email already registered'));
