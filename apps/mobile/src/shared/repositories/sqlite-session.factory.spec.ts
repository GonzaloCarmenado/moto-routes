import { describe, it, expect, vi, afterEach } from 'vitest';

const loadMock = vi.fn();

vi.mock('@tauri-apps/plugin-sql', () => ({
  default: { load: loadMock },
}));

import { createSqliteSessionDb } from './sqlite-session.factory.js';

describe('createSqliteSessionDb', () => {
  afterEach(() => {
    vi.clearAllMocks();
  });

  it('creates the connection and delegates execute/select to the Database instance', async () => {
    const executeMock = vi.fn().mockResolvedValue({ rowsAffected: 1 });
    const selectMock = vi.fn().mockResolvedValue([{ id: 1 }]);
    loadMock.mockResolvedValue({ execute: executeMock, select: selectMock });

    const db = await createSqliteSessionDb();

    expect(loadMock).toHaveBeenCalledWith('sqlite:moto-routes.db');
    await db.execute('INSERT OR REPLACE INTO session VALUES (?)', ['x']);
    expect(executeMock).toHaveBeenCalledWith('INSERT OR REPLACE INTO session VALUES (?)', ['x']);
    await db.select('SELECT * FROM session');
    expect(selectMock).toHaveBeenCalledWith('SELECT * FROM session', undefined);
  });

  it('throws a descriptive error when the Tauri SQL plugin is not available', async () => {
    loadMock.mockRejectedValue(new Error('plugin not registered'));

    await expect(createSqliteSessionDb()).rejects.toThrow(
      'SqliteSessionRepository: Tauri SQL plugin not available.',
    );
  });
});
