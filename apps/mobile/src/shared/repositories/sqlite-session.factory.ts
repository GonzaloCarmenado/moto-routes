 ...
 * Reutiliza la misma base de datos de rutas/perfil/fotos (moto-routes.db).
 * Solo funciona dentro de la WebView de Tauri (Desktop/Android).
 * En entorno navegador lanza un error descriptivo.
 */
export async function createSqliteSessionDb(): Promise<SqlDb> {
  try {
    ...
    throw new Error(
      ...
      + 'Use MemorySessionRepository for development/testing.',
    );
  }
}
