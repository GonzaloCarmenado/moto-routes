...
func TestPostgresRouteStore_UpsertCalledTwiceReplacesDataWithoutDuplicating(t *testing.T) {
	...
	if err := store.Upsert(context.Background(), userID, updated); err != nil {
...
func TestPostgresRouteStore_UpsertRejectsRouteExceedingMaxPoints(t *testing.T) {
	...
	if err := store.Upsert(context.Background(), userID, detail); err != ErrTooManyPoints {
...
func TestPostgresRouteStore_UpsertNeverOverwritesAnotherUsersRoute(t *testing.T) {
	...
	if err := store.Upsert(context.Background(), owner, detail); err != nil {
		t.Fatalf("unexpected error seeding owner's route: %v", err)
	}

	tampered := detail
	tampered.Duration = 999
	if err := store.Upsert(context.Background(), attacker, tampered); err != ErrRouteOwnedByAnotherUser {
...
func TestPostgresRouteStore_ListByUserReturnsOnlySummariesForThatUser(t *testing.T) {
	...
	if err := store.Upsert(context.Background(), userA, sampleDetail("55555555-5555-5555-5555-555555555555")); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if err := store.Upsert(context.Background(), userB, sampleDetail("66666666-6666-6666-6666-666666666666")); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
