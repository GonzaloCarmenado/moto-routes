...
- **Feature activo**: Ninguno. `grabacion-rutas` Fase 2 cerrado (AC-024 verificado en dispositivo real). `deuda-tecnica-auditoria` commiteado, mergeado y pusheado a `origin/master`. ISSUE-001 del review de `mejoras-fotos-mapa` (verificación visual en Android real + migración ALTER TABLE) **resuelto (2026-07-31)**, confirmado por el usuario.
