...
- **Fase**: APK Android compilado y funcional, con persistencia de rutas y fotos verificada en dispositivo real. `mejoras-tecnicas` (PR #44) y `mejoras-usabilidad` (PR #45) mergeadas a `master`, ramas borradas. `mejoras-fotos-mapa` terminada (32/32 AC, review APPROVED WITH MINOR ISSUES) en `feature/mejoras-fotos-mapa`, pusheada pero **sin PR creado ni mergeada todavía**.
- **Feature activo**: Ninguno en desarrollo activo. `mejoras-fotos-mapa` está lista salvo por 2 issues de severidad baja no bloqueantes (ver detalle en `specs/features/mejoras-fotos-mapa.review.md` y el resumen de "Próximo hito" más abajo) y falta abrir/mergear su PR. Fotos de ruta (`specs/features/fotos-ruta.md`) está en 33/33 AC; ver [[fotos-ruta-review]].
...
- **`mejoras-fotos-mapa` — Paso 7/7 completado (2026-07-26), plan íntegro terminado, 32/32 AC de la spec marcadas**: en `feature/mejoras-fotos-mapa`, último paso del plan — trazado SVG en `<route-list>` con backfill perezoso. `route-list.transform.ts#buildPolylineSvgPath(polyline, width, height)` construye el `d` de un `<path>` normalizando lat/lng de forma independiente por eje (silueta, no proyección cartográfica fiel) con Y invertida (norte arriba); `route-list-polyline.service.ts#ensurePreviewPolyline(repository, route)` es el único gatekeeper de "ya tiene trazado (`previewPolyline !== null`, aunque sea `[]`) / hay que calcularlo con `getPointsByRouteId` + `simplifyPolyline` + `updatePreviewPolyline` / no hay `route_points`, devuelve `null`". `route-list.element.ts#buildThumb()` renderiza el `<svg>` si hay `previewPolyline` disponible, o el placeholder existente lanzando el backfill sin `await` (nunca bloquea el render); al resolver, sustituye el `.thumb` de esa tarjeta concreta vía una referencia cerrada (closure) al propio nodo `card` y muta `route.previewPolyline` in-place (mismo objeto que vive en `this._routes`) para que una re-renderización posterior no repita el backfill. Al escribir el test de este backfill se descubrió una condición de carrera preexistente en el helper de test `createList()` (el setter de `repository` + `connectedCallback` disparan `fetchAndRender()` dos veces al montar en ese orden) — invisible en tests anteriores por ser idempotentes, pero aflorada al introducir un mock `mockReturnValueOnce` en `getPointsByRouteId`; se resolvió en el test montando el elemento antes de asignar `repository`, sin tocar el componente (el doble fetch en sí no rompe nada funcionalmente, solo hacía el test no determinista). Con esto, `specs/features/mejoras-fotos-mapa.md` queda en 32/32 AC y el plan de 7 pasos completo. `review-agent` ya se ejecutó (ver [[mejoras-fotos-mapa-review]]): veredicto **APPROVED WITH MINOR ISSUES**, quality gates verificados directamente por el propio agente (346/346 tests, cobertura 94.44%, `tsc`/ESLint/`cargo fmt`/`clippy`/`test` limpios), los dos puntos de mayor riesgo (preservar `previewPolyline` en el upsert de `MemoryRouteRepository`, migración `PRAGMA table_info`+`ALTER TABLE` de `SqliteRouteRepository`) confirmados con tests de regresión reales. Quedan 2 issues de severidad baja sin resolver: ISSUE-001 (verificación visual/Cypress de pestañas/cuadrícula/popup/trazado, y confirmar en Android real que el `ALTER TABLE` no rompe la BBDD existente — mismo patrón que ISSUE-001 de `mejoras-usabilidad`, ya cerrado con éxito) e ISSUE-002 (sin ADR nuevo, evaluado como razonable, sin acción requerida). Falta abrir el PR y mergear `feature/mejoras-fotos-mapa` a `master`.
