...
- **Próximo hito**: Dos specs de mejoras recién escritas y pendientes de PLAN — [[mejoras-tecnicas]] (`specs/features/mejoras-tecnicas.md`: unificar `BaseElement`, constantes de eventos, quitar CSS inline, tokens semánticos, deduplicar servicios de foto — refactor sin cambio de comportamiento) y [[mejoras-usabilidad]] (`specs/features/mejoras-usabilidad.md`: módulo de feedback compartido toast+confirm-dialog, guardar/descartar al parar ruta, confirmaciones destructivas, galería/visor de fotos compartidos también en la pantalla de grabación). Decidido: dos specs separadas + módulo de feedback compartido (no per-pantalla). Parte del backlog de `fotos-ruta` se absorbe aquí (el swipe del visor, AC-020, entra como AC-015 de usabilidad); siguen pendientes fuera de alcance el popup de marcador (AC-015 fotos-ruta), la desagrupación de clusters al zoom (AC-018) y el test de EXIF con GPS real (AC-005/006)
