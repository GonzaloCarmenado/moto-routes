...
- **Sesión 2026-07-27 (2) — feature `mejoras-guardado-rutas` (18/18 AC, 11/11 pasos del plan)**: implementados los 3 cambios de la spec (ver [[mejoras-guardado-rutas]]): (1) nombre de ruta al guardar, (2) notas editables en `<route-detail>`, (3) fix visual del punto descentrado en el botón "Grabar". `Route`/`CreateRoute` (`route.types.ts`) ganan `name: string | null` y `notes: string | null` (`CreateRoute.name` opcional — la regla de "nunca vacío" vive en `cockpit-stop.service.ts`, no en el tipo); `IRouteRepository` gana `updateNotes(routeId, notes)`, mismo patrón `ALTER TABLE ... ADD COLUMN` bajo demanda que `preview_polyline` (ADR-020) para migrar instalaciones existentes sin pérdida de datos. Nuevo diálogo específico `cockpit-save-route-dialog` (`src/cockpit/`) sustituye al `confirmDialog()` genérico en el flujo de parada — campo de nombre embebido con límite de 100 caracteres, réplica deliberada (no extracción) del focus-trap/overlay de `<confirm-dialog>` para no tocar ese componente compartido ni sus 3 consumidores existentes; decisión registrada en [[ADR-025]]. `sanitizeRouteName`/`buildDefaultRouteName` (`cockpit.transform.ts`) calculan el nombre final: texto del usuario recortado y truncado, o "Ruta {fecha}, {hora}" (usando `metadata.date`, el momento de parada) si se deja vacío. `<route-list>`/`<route-detail>` muestran `route.name` con fallback al cálculo "Ruta {fecha}" ya existente para rutas antiguas/interrumpidas (`name: null`). El editor de notas (`route-detail-notes.ts`, extraído de `route-detail.element.ts` para no superar el límite de tamaño de archivo) persiste vía `updateNotes()` con un botón "Guardar nota" explícito — guardar vacío borra la nota sin pedir confirmación (edición normal, no destructiva). Fix del punto de la nav-bar: `.nav-item--record` pasa de `position:absolute; top:20px` (no descontaba el `padding` del botón) a `display:grid` con `::before`/`.record-dot` compartiendo la misma `grid-area`, centrado estructural. Lección de tooling: los imports `?inline` de CSS resuelven a cadena vacía bajo Vitest en este proyecto (`test.css` no está activado en `vitest.config.ts`) — el test de guardia sobre el CSS fuente del punto de la nav-bar usa `fs.readFileSync` directo en vez del import `?inline` real. 391/391 tests, cobertura ~94.9% statements / ~93% functions / ~89% branches (todas por encima del umbral 80%), ESLint y `tsc` limpios, `vite build` exitoso. No se ha tocado `src-tauri` en esta sesión. `review-agent`: **APPROVED**, 18/18 AC verificados de primera mano (391/391 tests, cobertura confirmada, `<confirm-dialog>` compartido con cero cambios confirmado por `git diff`); único punto abierto de severidad baja, ya anticipado por el plan: falta verificación visual real del punto de la nav-bar (jsdom no calcula layout) — ver [[mejoras-guardado-rutas-review]]. **Refinamiento post-review (mismo día)**: el usuario pidió que si una ruta ya tiene nota se muestre en modo vista (texto integrado, sin borde) con icono de editar arriba a la derecha, en vez de mostrar siempre el `<textarea>` editable — nuevo AC-019, `saveRouteNote` ahora devuelve `Promise<boolean>` para que el panel sepa cuándo volver a modo vista tras guardar. 392/392 tests, cobertura 94.94%. Recompilado e instalado en dispositivo real para revisión visual (nav-bar + notas). **Confirmado visualmente por el usuario** (punto de la nav-bar centrado, modo vista de notas correcto) y **mergeado a `master`** (PR #48, fast-forward, rama `feature/mejoras-guardado-rutas` borrada) en esta misma sesión.
- **Sesión 2026-07-27 — fix: GPS no grababa en segundo plano (Modo Invisible era cosmético)**: usuario reportó una ruta guardada como línea recta (solo 2 puntos GPS, 5.3s de diferencia) pese a haber bloqueado el móvil durante el trayecto. Investigación (ver [[grabacion-rutas-review]], sección "Cierre AC-012/AC-013"): `RecordingService.kt` (foreground service Android) y `MainActivity.startRecordingService()/stopRecordingService()` ya existían y funcionaban a nivel nativo, pero nada los invocaba de verdad — los comandos Tauri `start_foreground_service`/`stop_foreground_service` eran stubs (`log::info!` + `Ok(())`) sin ningún JNI real, y `setInvisibleMode()` en `cockpit.service.ts` solo cambiaba un booleano de UI. Fix en rama `feature/fix-gps-background` (PR #47, mergeado a `master` por fast-forward, rama borrada): puente Rust↔Kotlin real vía el mecanismo oficial de mobile plugins de Tauri 2 (ver sección "Tauri Mobile Plugins" más abajo), nuevo `ForegroundServiceProvider` inyectable en `cockpit.service.ts` (mismo patrón que `GpsProvider`), wiring en `cockpit.element.ts`. De paso se detectaron y arreglaron 3 tests obsoletos en `route-map.element.spec.ts` que testeaban un popup de foto ya eliminado en el commit `36afc4f` (spec drift puro, sin tocar producción) — bloqueaban el reporte de cobertura porque Vitest no lo genera si hay tests en rojo (`reportOnFailure: false` por defecto). Verificado: 352/352 tests (94.45% cobertura), `cargo fmt`/`clippy`/`build` limpios, y build Android real (`pnpm tauri android build`) compiló y enlazó el plugin Kotlin+JNI sin errores, instalado en dispositivo real. **Cambio de producto en la misma sesión**: el usuario decidió retirar el toggle "Modo Invisible" por completo (con el fix seguía siendo opt-in — fácil de olvidar activarlo antes de bloquear el móvil) y que la grabación en segundo plano sea automática siempre. Se eliminó `invisibleMode` de `CockpitState`, `setInvisibleMode()`, el botón (`buildInvisibleToggle`) y su CSS; `cockpit.service.ts` ahora arranca/para el foreground service incondicionalmente en `startRecording`/`prepareStop`. AC-011 de `grabacion-rutas.md` reescrita, AC-012/014/015 retiradas — ver detalle en [[grabacion-rutas-review]] sección "Retirada del toggle". 346/346 tests, cobertura 94.56%. **Verificado en dispositivo real por el usuario**: grabación con pantalla bloqueada funciona — notificación persistente y puntos GPS siguen llegando sin ninguna acción manual. Mergeado a `master` (PR #47) en esta misma sesión.
- **Fase**: APK Android compilado y funcional, con persistencia de rutas y fotos verificada en dispositivo real. `mejoras-tecnicas` (PR #44), `mejoras-usabilidad` (PR #45), `mejoras-fotos-mapa` (PR #46), `fix-gps-background` (PR #47) y `mejoras-guardado-rutas` (PR #48) mergeadas a `master`, ramas borradas. `mejoras-fotos-mapa` terminada (32/32 AC, review APPROVED WITH MINOR ISSUES) e integrada. Queda pendiente ISSUE-001 del review (verificación visual en Android real + confirmar migración ALTER TABLE en dispositivo).
- **Feature activo**: Ninguno en desarrollo activo. `mejoras-guardado-rutas` (nombre de ruta al guardar, notas editables, fix nav-bar) mergeada a `master` (PR #48) — ver [[mejoras-guardado-rutas-review]]. Fotos de ruta (`specs/features/fotos-ruta.md`) está en 33/33 AC; ver [[fotos-ruta-review]]. Pendiente ISSUE-001 del review de `mejoras-fotos-mapa` (verificación visual en Android real + migración ALTER TABLE).
