...
- **Feature activo**: `mejoras-visuales-mapa` (ver sesión 2026-08-01 arriba) — Pasos 1-6 commiteados, Pasos 8-9 implementados en el working tree (sin commitear) y con APK ya instalado en el dispositivo `75fe536b`, pendiente de verificación visual del usuario antes de commitear e invocar `review-agent`. `grabacion-rutas` Fase 2 cerrado (AC-024 verificado en dispositivo real). `deuda-tecnica-auditoria` commiteado, mergeado y pusheado a `origin/master`. ISSUE-001 del review de `mejoras-fotos-mapa` (verificación visual en Android real + migración ALTER TABLE) **resuelto (2026-07-31)**, confirmado por el usuario.
