...
- **Próximo hito**: Sin spec activa. Pendientes conocidos fuera de alcance de las specs cerradas: en `fotos-ruta` — popup de marcador individual en el mapa (AC-015), desagrupación de clusters al hacer zoom (AC-018), test de EXIF con GPS real (AC-005/006); en `mejoras-usabilidad` — verificar en dispositivo Android real que el pragma de foreign_keys cascada de verdad `route_points`/`route_stops` (ISSUE-001 de su review), y homogeneizar más los estados de carga si surge un tercer caso de uso (ISSUE-002)
