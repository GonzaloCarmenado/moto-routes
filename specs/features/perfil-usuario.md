...
### Bloque 4 — Mejoras post-cierre (buscador de marca, precarga y feedback de guardado)
Añadido el 2026-08-02 tras verificación en dispositivo Android real, a partir de feedback directo del usuario sobre el flujo de edición de vehículo y el guardado de perfil — mismo patrón ya usado en `mejoras-visuales-mapa.md` (Pasos 9/10, corrección directa sobre la spec ya cerrada, sin reabrir `plan-agent`).
- [x] AC-039: Al abrir "Editar vehículo" sobre un vehículo ya guardado, las marcas del tipo guardado (y los modelos de la marca guardada, si la carga de marcas tuvo éxito) se cargan automáticamente, sin que el usuario tenga que tocar el selector de tipo — bug real corregido: antes, editar un vehículo sin cambiar el tipo dejaba el campo "Marca" sin cargar nunca. No viola AC-024: abrir "Editar vehículo" ya es entrar en el flujo de edición, la única condición de esa AC.
- [x] AC-040: El campo "Marca" es un buscador de texto (no un `<select>` nativo) que filtra en el cliente, en tiempo real e insensible a mayúsculas, la lista de marcas ya devuelta por vPIC — la API pública NHTSA vPIC no soporta ningún filtro server-side en `GetMakesForVehicleType`, siempre devuelve la lista completa (que incluye fabricantes muy minoritarios).
- [x] AC-041: Sin texto de búsqueda, una lista curada de fabricantes de moto/coche ampliamente conocidos o internacionales (si están presentes en el resultado de vPIC para el tipo elegido) se muestra primero, en orden alfabético, seguida del resto de marcas devueltas por la API, también en orden alfabético — nunca se oculta ninguna marca devuelta por la API, solo se prioriza el orden. Con texto de búsqueda, se muestran todas las coincidencias (curadas o no) en orden alfabético, ignorando la priorización.
- [x] AC-042: Al pulsar "Guardar" en el modal "Editar perfil", mientras la foto (si se cambió) se escribe a disco y el perfil se persiste, el botón "Guardar" muestra un indicador de carga (spinner) y los demás controles del modal (nombre, "Cambiar foto", "Cancelar") quedan deshabilitados — evita que operaciones lentas (p. ej. guardar una foto grande) parezcan una app colgada.

### Bloque 5 — Filtrado real de modelos por tipo de vehículo
Añadido el 2026-08-02 tras reporte del usuario: al elegir un fabricante que vende tanto motos como coches (p. ej. Honda) y tipo "Moto", el select de modelo mostraba también modelos de coche. Investigado contra la API real: `GetModelsForMake/{make}` (usado hasta ahora) nunca informa el tipo de vehículo del modelo — verificado con `curl` contra `vpic.nhtsa.dot.gov` — así que el filtrado por tipo era, en la práctica, un no-op documentado desde el Paso 5 (ver decisión de diseño #10 del plan). Se encontró el endpoint real que sí filtra: `GetModelsForMakeIdYear/makeId/{id}/modelyear/{year}/vehicletype/{type}` (confirmado con datos reales: 57 modelos de Honda para 2026 tipo moto, todos con `VehicleTypeName: "Motorcycle"`, ninguno de coche).
- [x] AC-043: Al consultar los modelos de una marca para un tipo de vehículo dado, solo se muestran modelos de ese tipo — nunca modelos de otro tipo del mismo fabricante (p. ej. elegir Honda + Moto nunca muestra "Civic" o "Accord"). Se usa `GetModelsForMakeIdYear` con el año actual; si esa consulta no devuelve ningún modelo (marca discontinuada o sin datos para ese año concreto), se recurre a `GetModelsForMake` sin filtrar antes que no mostrar ningún modelo.
