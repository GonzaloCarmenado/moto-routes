...
- [x] AC-005: Al capturar una foto nueva con la cámara, si la imagen contiene metadatos EXIF con coordenadas GPS, se usan esas coordenadas como ubicación de la foto. _(`extractPhotoLocation` intenta EXIF vía `exifr` primero. Cerrado el gap real: faltaba verificar el camino "con EXIF" a través del pipeline completo de persistencia, no solo la función pura — test: photo-persist.service.spec.ts — "persists the EXIF GPS coordinates (not the fallback)...")_
- [x] AC-006: Al seleccionar una foto de la galería, si la imagen contiene metadatos EXIF con coordenadas GPS, se usan esas coordenadas. Si no contiene GPS, se intenta vincular al punto GPS más cercano en el tiempo de la ruta activa. _(mismo camino/test que AC-005 — `extractPhotoLocation` no distingue cámara de galería)_
- [x] AC-007: Si una foto no tiene GPS en EXIF y no hay ruta activa (ej: en detalle de ruta guardada sin grabación en curso), se usa el centroide de la ruta como ubicación de la foto. _(Reescrita para reflejar la decisión ya tomada e implementada — ver [[ADR-024]] y AC-013, con la que antes contradecía en vez de ser coherente)_
...
- [x] AC-015: Al pulsar un marcador de foto en el mapa, se muestra un popup/tooltip con la miniatura de la foto correspondiente. _(`route-map.element.ts#showPhotoPopup` usa `maplibregl.Popup` con la `objectUrl` de la foto; de paso se corrigió un bug latente donde las fotos asignadas antes de que el mapa terminara de cargar nunca llegaban a pintarse — `renderPhotoMarkers()` ahora también se dispara en el evento `load`. Test: route-map.element.spec.ts)_
...
- [x] AC-018: Al hacer zoom en el mapa, los clusters se desagrupan progresivamente mostrando los marcadores individuales cuando el nivel de zoom permite distinguirlos visualmente. _(`photoClusterRadiusForZoom()` convierte los 50m de `PHOTO_CLUSTER_RADIUS_METERS` en un radio que se encoge/crece exponencialmente según el zoom respecto a un zoom de referencia (15); `route-map.element.ts` escucha `zoomend` y recalcula los marcadores. Test: route-map.element.spec.ts, route-map-photos.spec.ts)_
